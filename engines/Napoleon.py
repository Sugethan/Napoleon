...
class node:
    ...
    def comp_usb(self, exploration):
        self.sons_ucb = self.player * ((1-exploration) * self.sons_v + self.sons_normalized_value * exploration) +  self.sons_p * np.sqrt(self.father.sons_visits[self.index]) / (1 + self.sons_visits)
    ...
    def update(self, exploration: float, result: int):
        ...
        if self.father == None:
            ...
            if self.sons:
                ...
                self.sons_ucb = self.player * ((1-exploration) * self.sons_v + self.sons_normalized_value * exploration) +  self.sons_p * np.sqrt(self.visits) / (1 + self.sons_visits) 
    ...
    def array_builder(self):
        ...
        unique_values = np.array([-10, -9, -5, -4, -3, -1, 0, 1, 3, 4, 5, 9, 10])
        ...
        channels = np.concatenate((channels, array.reshape(8,8,1)), axis=2)
    ...
    def blooming(self):
        ...
        f = np.full((l, 8, 8, 14), a)

        self.sons_v, self.sons_p = self.model.predict([self.sons_arrays, p, f], verbose = 0)
